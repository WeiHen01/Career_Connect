...
class _UserEditState extends State<UserEdit> {
  ...
  Future<void> getUser() async {
    ...
    WebRequestController req = WebRequestController(
        path: "/inployed/user/jobseeker/${widget.id}",
  ...
  Future<void> updateAccount() async{
    ...
    if (passwordctrl.text != null && passwordctrl.text.isNotEmpty) {
      requestBody["userpassword"] = md5.convert(utf8.encode(passwordctrl.text)).toString();
      password = passwordctrl.text;
    }

    final prefs = await SharedPreferences.getInstance();
    String? server = prefs.getString("localhost");
    WebRequestController req = WebRequestController(path: "/inployed/user/update/${widget.id}",
        server: "http://$server:8080");
