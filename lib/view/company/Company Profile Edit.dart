...
class CompanyEdit extends StatefulWidget {
  const CompanyEdit({required this.id, required this.company});
  final int id, company;
...
class _CompanyEditState extends State<CompanyEdit> {
  ...
  Future<void> getUser() async {
    ...
    WebRequestController req = WebRequestController(
        path: "/inployed/user/company/${widget.id}",
    ...
    if(req.status() == 200) {
      ...
      Navigator.pushAndRemoveUntil(context,
          MaterialPageRoute(builder: (context)
            => CompanyHomeNavi(username: username, id: id,
              tabIndexes: 4, company: widget.company,)),
              (route) => false
      );
    }
    else{
  ...
  late Uint8List? _images; // Default image URL
